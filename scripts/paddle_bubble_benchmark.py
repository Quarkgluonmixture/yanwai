...
def run(args, fixtures):
    ...
    for fixture in fixtures:
        ...
        for index,line in enumerate(line_images):
            line.save(args.output / 'crops' / f'{fixture["Name"]}-line{index}.png')
        ...
        (args.output / 'rows.json').write_text(json.dumps(rows,ensure_ascii=False,indent=2),encoding='utf-8')
    ...
    (args.output/'results.json').write_text(json.dumps(dict(metadata=metadata,summary=summary,rows=rows),ensure_ascii=False,indent=2),encoding='utf-8')
...
if __name__ == '__main__':
    ...
    if args.snapshot_baseline:
        ...
        fixtures=prepare(args.manifest,args.output)
